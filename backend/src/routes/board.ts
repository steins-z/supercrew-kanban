...
boardRouter.get("/multi-branch", async (c) => {
  ...
  const branchPatternParam = c.req.query("branch_pattern");

  // Parse comma-separated patterns, default to user/* and feature/*
  const branchPatterns = branchPatternParam
    ? branchPatternParam
        .split(",")
        .map((p) => p.trim())
        .filter((p) => p.length > 0)
  ...
  try {
    ...
    const branches = await scanner.discoverBranches(branchPatterns);
